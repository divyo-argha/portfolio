  ...
  "git-user": [
    {
      ...
      body: [
        ...
        "It manages passphrase-encrypted SSH keys per identity, applies gitconfig overrides scoped to a directory, supports commit signing, and installs a pre-commit hook that verifies the active identity matches the intended one before a commit lands — catching the \"pushed to the wrong account\" mistake before it happens.",
  ...
  "claude-user": [
    {
      kind: "prose",
      body: [
        "Running multiple Claude Code accounts side by side — work, personal, research — usually means repeated logins and logouts. claude-user isolates each account's session so they can run simultaneously across different terminals, switched with a single command or through an interactive TUI.",
      ],
    },
    {
      kind: "findings",
